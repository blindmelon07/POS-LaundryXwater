...
function ItemForm({ item, onClose }: { item?: InventoryItem; onClose: () => void }) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        if (item) {
            put(`/inventory/${item.id}`, { onSuccess: onClose });
        } else {
            post('/inventory', { onSuccess: onClose });
        }
    };
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                    ...
                    <Input
                        ...
                        placeholder="e.g. 19L Slim Container"
                    />
...
function AdjustForm({ item, onClose }: { item: InventoryItem; onClose: () => void }) {
    ...
    const newQty =
        data.type === 'adjustment'
            ...
            : data.type === 'add'
              ? item.quantity + (parseFloat(data.quantity) || 0)
    ...
    return (
        <form
            onSubmit={(e) => {
                e.preventDefault();
                post(`/inventory/${item.id}/adjust`, { onSuccess: onClose });
            }}
            className="flex flex-col gap-4"
        >
            <div className="rounded-lg bg-muted/50 px-4 py-3 text-sm">
                Adjusting <strong>{item.name}</strong> — current:{' '}
            ...
            {data.quantity && (
                <div className="flex items-center gap-2 rounded-lg bg-muted/50 px-4 py-2.5 text-sm">
                    ...
                    <span
                        className={`font-bold ${newQty <= (item.min_quantity ?? 0) ? 'text-red-600' : 'text-emerald-700'}`}
                    >
                        {newQty} {item.unit}
                    </span>
                </div>
            ...
            <div>
                ...
                <Input
                    ...
                    placeholder="e.g. Purchased from supplier, Damaged, Physical count"
                />
    ...
    return (
        <div className="flex flex-col gap-3">
            <p className="text-sm text-muted-foreground">
                Recent adjustments for <strong>{item.name}</strong>
            </p>
...
export default function InventoryIndex({ items, low_stock_count, total_value }: Props) {
    ...
    const deleteItem = (id: number, name: string) => {
        if (confirm(`Delete "${name}"? This cannot be undone.`)) router.delete(`/inventory/${id}`);
    };
        ...
        <>
                ...
                <div className="flex items-center gap-3">
                    <div>
                        ...
                        <div className="mt-1 flex items-center gap-3 text-sm text-muted-foreground">
                            <span>
                                Total value:{' '}
                                <span className="font-semibold text-foreground">{fmt(total_value)}</span>
                            </span>
                            {low_stock_count > 0 && (
                ...
                {low_stock_count > 0 && (
                    <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
                        ...
                        <p className="text-sm font-medium text-amber-800">
                            {low_stock_count} item{low_stock_count !== 1 ? 's are' : ' is'} running low on stock.
                            Check and restock soon.
                        </p>
                    </div>
                )}
                                    ...
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody className="divide-y">
                                            {categoryItems.map((item) => (
                                                <tr
                                                    key={item.id}
                                                    className={`transition-colors hover:bg-muted/40 ${item.is_low_stock ? 'bg-amber-50/50' : ''}`}
                                                >
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <span
                                                            className={`font-bold tabular-nums ${item.is_low_stock ? 'text-amber-700' : ''}`}
                                                        >
                                                            {item.quantity} {item.unit}
                                                        </span>
                                                    </td>
                ...
                {items.length === 0 && (
                    <Card>
                        <CardContent className="flex flex-col items-center gap-2 py-16 text-center text-muted-foreground">
                            ...
                            <p className="text-sm">Add items to start tracking stock levels</p>
            ...
            <Dialog open={!!historyItem} onOpenChange={(o) => !o && setHistoryItem(null)}>
                <DialogContent>
                    ...
                    {historyItem && (
                        <AdjustmentHistory item={historyItem} onClose={() => setHistoryItem(null)} />
                    )}
                </DialogContent>
            </Dialog>
        </>
